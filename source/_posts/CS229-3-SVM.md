...
回顾 dual form 预测函数（第 8 节）：

$$h_{w,b}(x)=g\Bigl(\sum_{i=1}^m\alpha_i y^{(i)}\langle x^{(i)},x\rangle+b\Bigr)$$

预测只依赖内积 $\langle x^{(i)},x\rangle$。但内积只能表达线性关系——对于非线性可分的数据，直接在原始空间用内积分类效果很差。

Kernel 的思路：将样本隐式映射到高维特征空间，用高维空间的内积替代原始内积。定义 $K(x,z)=\phi(x)^T\phi(z)$，其中 $\phi$ 是特征映射，但**无需显式计算 $\phi(x)$**，只需计算 kernel 函数值。

将预测函数中的内积替换为 kernel：

$$h(x)=g\Bigl(\sum_{i=1}^m\alpha_i y^{(i)}K(x^{(i)},x)+b\Bigr)$$

这就是 **Kernel SVM 的核心公式**。注意公式里不需要显式写出 $\phi(x)$，只需要计算 kernel——这正是 kernel trick 的关键优势。
